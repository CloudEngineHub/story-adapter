...
parser.add_argument('--story', default=story6, nargs='+', type=str)
...
storyiter = StoryIterXL(pipe, image_encoder_path, ip_ckpt, device)
...
for i, text in enumerate(prompts):
    images = storyiter.generate(pil_image=None, num_samples=1, guidance_scale=8.0, num_inference_steps=4, seed=seed,
...
for i, scale in enumerate(scales):
    ...
    for y, text in enumerate(prompts):
        image = storyiter.generate(pil_image=images, num_samples=1, guidance_scale=8.0, num_inference_steps=4, seed=seed,
